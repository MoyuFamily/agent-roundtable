...
def test_register_agent(setup):
    core, db = setup
    result = handle_tool_call(core, db, "roundtable_register_agent", {
...
def test_list_agents(setup):
    core, db = setup
    handle_tool_call(core, db, "roundtable_register_agent", {
        "agent_id": "agent-a", "platform": "claude-code",
    })
    handle_tool_call(core, db, "roundtable_register_agent", {
        "agent_id": "agent-b", "platform": "cursor",
    })
...
def test_create_discussion_and_invite(setup):
    core, db = setup
    handle_tool_call(core, db, "roundtable_register_agent", {
        "agent_id": "coordinator-1", "platform": "claude-code",
    })
    handle_tool_call(core, db, "roundtable_register_agent", {
        "agent_id": "participant-1", "platform": "cursor",
    })

    result = handle_tool_call(core, db, "roundtable_create", {
...
def test_accept_invite_and_speak(setup):
    ...
    handle_tool_call(core, db, "roundtable_register_agent", {
        "agent_id": "dev", "platform": "cursor",
    })

    create_result = handle_tool_call(core, db, "roundtable_create", {
    ...
    accept = handle_tool_call(core, db, "roundtable_accept_invite", {
        "discussion_id": disc_id, "agent_id": "dev",
    })
    assert accept["status"] == "accepted"

    speak_result = handle_tool_call(core, db, "roundtable_speak", {
...
def test_decline_invite(setup):
    ...
    handle_tool_call(core, db, "roundtable_register_agent", {
        "agent_id": "busy-agent", "platform": "windsurf",
    })
    ...
    decline = handle_tool_call(core, db, "roundtable_decline_invite", {
        "discussion_id": disc_id, "agent_id": "busy-agent",
    })
...
def test_wait_for_turn(setup):
    ...
    handle_tool_call(core, db, "roundtable_speak", {
        "discussion_id": disc_id, "participant": "coordinator",
        "content": "Opening.",
    })

    turn = handle_tool_call(core, db, "roundtable_wait_for_turn", {
        "discussion_id": disc_id, "agent_id": "alice",
    })
    assert turn["your_turn"] is True
    assert turn["next_speaker"] == "alice"

    turn_bob = handle_tool_call(core, db, "roundtable_wait_for_turn", {
        "discussion_id": disc_id, "agent_id": "bob",
    })
...
def test_full_discussion_flow(setup):
    ...
    handle_tool_call(core, db, "roundtable_speak", {
        "discussion_id": disc_id, "participant": "alice",
        "content": "I think we should use PostgreSQL.",
    })

    result = handle_tool_call(core, db, "roundtable_speak", {
        "discussion_id": disc_id, "participant": "bob",
    ...
    end = handle_tool_call(core, db, "roundtable_end", {
        "discussion_id": disc_id,
...
def test_list_discussions(setup):
    core, db = setup
    handle_tool_call(core, db, "roundtable_create", {
        "topic": "Discussion 1",
        "participants": [
            {"profile": "a", "role": "Dev"},
            {"profile": "b", "role": "PM"},
